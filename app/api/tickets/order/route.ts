...
function generateBarcodeData(orderNum: string, seq: number): string {
  return `${orderNum}-${String(seq).padStart(3, '0')}`;
}
...
async function issueTickets(
  ...
  order: {
    ...
    tier_id: string; quantity: number; order_number: string;
  },
  tier: { section: string; is_seated: boolean }
) {
  const tickets = [];
  for (let i = 0; i < order.quantity; i++) {
    ...
    tickets.push({
      ...
      qr_data: generateQrData(ticketNum),
      barcode_data: generateBarcodeData(order.order_number, i + 1),
      status: 'valid',
    });
  }
  ...
  const { data: tierData } = await supabase
    .from('ticket_tiers').select('sold_quantity').eq('id', order.tier_id).single();
  if (tierData) {
    await supabase.from('ticket_tiers')
      .update({ sold_quantity: (tierData.sold_quantity || 0) + order.quantity })
      .eq('id', order.tier_id);
  }
}
...
export async function POST(req: NextRequest) {
  try {
    const {
      userId, eventId, tierId, quantity,
    ...
    if (paymentMethod === 'wallet') {
      ...
      await supabase.from('wallets').update({
        available_balance: wallet.available_balance - totalUsd,
      ...
      await supabase.from('wallet_transactions').insert({
        ...
        status: 'completed',
        reference: ref,
        description: `Ticket purchase (${quantity} ticket${quantity > 1 ? 's' : ''})`,
      });
      ...
      // Get tier info
      const { data: tierData } = await supabase
        .from('ticket_tiers').select('*').eq('id', tierId).single();
      ...
      await supabase.from('notifications').insert({
        ...
        message: `Your ${quantity} ticket${quantity > 1 ? 's' : ''} have been issued. Check My Tickets!`,
        metadata: { orderId: order.id, quantity }
      });
    ...
    const { data: order } = await supabase
      .from('ticket_orders')
      .insert({
        user_id: userId, event_id: eventId, tier_id: tierId,
        quantity, unit_price_usd: unitPriceUsd,
        total_price_usd: totalUsd,
        payment_method: 'paystack',
        payment_reference: reference,
        payment_status: 'pending',
        buyer_name: buyerName, buyer_email: buyerEmail,
        buyer_phone: buyerPhone, status: 'pending',
      })
      .select().single();
    ...
    const res = await fetch('https://api.paystack.co/transaction/initialize', {
      ...
      body: JSON.stringify({
        ...
        currency: 'KES',
        reference,
        callback_url: `${appUrl}/my-tickets?verify=${reference}`,
        metadata: {
