...
function Barcode({ data }: { data: string }) {
  const finalPattern = Array.from({ length: 80 }, (_, i) =>
    Math.sin(i * 0.7 + (data ? data.charCodeAt(i % data.length) : i) * 0.1) > 0.3 ? 2 : 1
  );
...
export default function TicketViewPage() {
  ...
  if (!data?.ticket || !data?.event) return (
    <div style={{ minHeight: '100dvh', background: '#060f1e', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontFamily: '-apple-system, sans-serif' }}>
      <div style={{ textAlign: 'center' }}>
        <p style={{ color: '#f87171', marginBottom: '12px' }}>Ticket not found</p>
        <Link href="/tickets/my-tickets" style={{ color: '#22c55e', textDecoration: 'none', fontSize: '14px', fontWeight: 600 }}>
          Back to My Tickets
        </Link>
      </div>
    </div>
  ...
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(ticket.qr_data || ticket.ticket_number)}&color=000000&bgcolor=FFFFFF&margin=10`;
    ...
    <div style={{
      ...
      <div style={{ background: '#0a1628', borderBottom: '1px solid #1a2740', padding: '14px 16px' }}>
        <div style={{ maxWidth: '480px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Link href="/tickets/my-tickets" style={{ color: '#9ca3af', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', fontWeight: 700 }}>
            ...
            <div style={{
              ...
              <div style={{ padding: '20px' }}>
                {[
                  ...
                  { l: 'Issued', v: ticket.created_at ? new Date(ticket.created_at).toLocaleString() : '' },
                ].map(item => item.v && (
